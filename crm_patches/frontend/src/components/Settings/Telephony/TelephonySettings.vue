      ...
      >
        <div class="flex flex-col gap-1">
          ...
          <span class="text-p-sm text-ink-gray-6">
            {{
              __(
                'Configure RingCentral credentials in Desk and set RingCentral Agent records for each rep.',
              )
            }}
          </span>
        </div>
        <Button
          :label="
            isEnabled('ringcentral')
              ? __('Update Configuration')
              : __('Configure')
          "
          @click="emit('updateStep', 'ringcentral-settings')"
        />
      </div>
    </div>
    <ErrorMessage
...
const defaultMediumOptions = computed(() => {
  ...
  if (isEnabled('twilio')) {
    options.push({ label: __('Twilio'), value: 'Twilio' })
  }
  if (isEnabled('exotel')) {
    options.push({ label: __('Exotel'), value: 'Exotel' })
  }
  if (isEnabled('ringcentral')) {
    options.push({ label: __('RingCentral'), value: 'RingCentral' })
  }
  return options
})
